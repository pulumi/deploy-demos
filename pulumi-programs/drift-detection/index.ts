...
import { RemoteWorkspace } from "@pulumi/pulumi/automation";
...
`;
  ...
  while (outstandingDeploymentIDs.length) {
    ...
    outstandingDeploymentIDs = outstandingDeploymentIDs.filter(
      (x) => completedDeployments.indexOf(x) === -1
    );
    console.log(
      `Finished polling deployments: ${completedDeployments.length} out of ${outstandingDeploymentIDs.length} complete.`
    );
  }

  if (driftedStacks.length) {
    console.log(
      `found ${driftedStacks.length} stacks that did not pass drift check`
    );
